...
export class UserManagerDB {
  ...
  async registerUser(signupForm) {
    try {
      ...
      logger.error(
        `register user error: Error al completar el registro: ${error}`
      );
      throw new Error(
        `register user error: Error al completar el registro: ${error}`
      );
    ...
    try {
      if (isGithubLogin) {
        const result = await this.model
          .findOne({ github_username: loginIdentifier })
          .lean();
        return result;
      } else {
        const result = await this.model
          .findOne({ email: loginIdentifier })
          .lean();
        return result;
      }
    } catch (error) {
  ...
  async getUsers() {
    try {
      ...
      throw new Error(
        `get users error: Error al obtener los usuarios: ${error}`
      );
    }
  }
  ...
  async getUserById(userId) {
    try {
      ...
      logger.error(
        `get user by id error: Error al obtener el usuario: ${error}`
      );
      throw new Error(
        `get user by id error: Error al obtener el usuario: ${error}`
      );
  ...
  async updateUser(userId, user) {
    try {
      const result = await this.model.findByIdAndUpdate(userId, user, {
        new: true,
      });
      ...
      logger.error(
        `update user error: Error al actualizar el usuario: ${error}`
      );
      throw new Error(
        `update user error: Error al actualizar el usuario: ${error}`
      );
