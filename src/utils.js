...
const checkUserValidFields = async (req, user, isUpdate = false) => {
  try {
    ...
    if (isUpdate) {
      ...
      if (
        existingEmail &&
        existingEmail._id.toString() !== updatedUser._id.toString()
      ) {
        return false;
      }
...
const checkProductValidFields = async (req, product, isUpdate = false) => {
  try {
    ...
    if (!isUpdate) {
      ...
      if (category !== "negra" && category !== "blanca") {
        return false;
      }
    }
    ...
    if (isUpdate) {
      ...
      if (
        existingCode &&
        existingCode._id.toString() !== updatedProduct._id.toString()
      ) {
        return false;
      }
      ...
      if (category && category !== "negra" && category !== "blanca") {
        return false;
      }
    }
...
const uploadProducts = multer({
  ...
  fileFilter: productsMulterFilter,
});
