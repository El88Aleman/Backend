...
export class UsersController {
  ...
  static modifyRole = async (req, res, next) => {
    try {
      ...
      res.json({
        status: "success",
        message: `El rol del usuario '${user.full_name}' fue modificado a '${user.role}' con éxito`,
      });
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      if (account_statement) {
        docs.push({
          ...
          reference: account_statement.filename,
        });
      }
      ...
      res.json({
        status: "error",
        error: `Error al actualizar los documentos del usuario: ${error.message}`,
      });
    }
  };
}
