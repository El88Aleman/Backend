      ...
      if (cart.products.length) {
        ...
        for (let i = 0; i < cart.products.length; i++) {
          ...
          if (productInCart.quantity <= productInfo.stock) {
            ...
            await ProductsService.updateProduct(productInfo._id, {
              stock: updateStock,
            });
          } else {
        ...
        const productsRejectedInCart = await CartsService.updateProductsInCart(
          cid,
          rejectedProducts
        );
        ...
        if (rejectedProducts.length) {
          if (ticketProducts.length) {
            ...
            res.status(201).json({
              message: `Compra realizada :) <br><br> Código de compra: ${newTicket.code} <br> Comprador: ${newTicket.purchaser} <br> Precio total: $${newTicket.amount}`,
