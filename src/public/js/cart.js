...
const tableCartContainer = document.querySelector(".table-cart-container");

if (tableCartContainer) {
  // Actualizar cantidad de los productos en el carrito
  const updateProductQuantityInCartBtn = document.querySelectorAll(
    ...
    btn.addEventListener("click", async (e) => {
      try {
        ...
        const productPrice = parseInt(
          e.currentTarget.getAttribute("data-product-price")
        );

        const quantityElement = e.currentTarget.parentElement.querySelector(
        ...
        // Actualizar precios
        const subtotalPriceElement =
          e.currentTarget.parentElement.parentElement.querySelector(
            ".subtotal-price-element"
          );
        ...
        const updatedSubtotalPrices = document.querySelectorAll(
          ".subtotal-price-element"
        );
        const newTotalPrice = Array.from(updatedSubtotalPrices).reduce(
          (acc, elem) => acc + parseInt(elem.innerText.replace("$", "")),
          0
        );
  ...
  const deleteProductInCartBtn = document.querySelectorAll(
  ...
  const deleteAllProductsInCartBtn = document.querySelector(
    ".delete-all-products-in-cart"
  );

  deleteAllProductsInCartBtn.addEventListener("click", async (e) => {
    try {
      const cartId = e.currentTarget.getAttribute("data-cart-id");
  ...
  // Finalizar compra
  const finishPurchaseBtn = document.querySelector(".finish-purchase");
...
window.addEventListener("load", () => {
  const deletedAllProductsInCart = JSON.parse(
    localStorage.getItem("deletedAllProductsInCart")
  );
...
window.addEventListener("load", () => {
  const deletedProductInCart = JSON.parse(
    localStorage.getItem("deletedProductInCart")
  );
