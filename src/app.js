...
socketServer.on("connection", async (socket) => {
  ...
  socket.on("addProduct", async (productInfo) => {
    try {
      const productToAdd = {
        ...productInfo,
        thumbnail: productInfo.imageName
          ? `${productInfo.code}-product-${productInfo.imageName}`
          : "noImgProduct-product.jpg",
      ...
      socketServer.emit("productsArray", {
        productsData: products,
        userSessionRole: userSessionRole,
        userSessionId: userSessionId,
      });
  ...
  socket.on(
    "deleteProduct",
    async ({ productId, productTitle, productOwner, userId, userRole }) => {
      try {
